...
def _relative_mse_loss(signal_train):
    ...
    def loss_function(y_true, y_pred):
        signal = y_true + tf.convert_to_tensor(np.mean(signal_train, axis=0))  # unpreproc signal to get the ampltiude
        reduced_amp = tf.math.reduce_max(tf.abs(signal), axis=1, keepdims=True)  # amplitude in units of standard eviation of signals
...
def train_emulator(emulator, signal_train, signal_val, par_train, par_val, epochs, initial_lr, lr_factor, lr_patience,
    ...
    :param par_train: numpy array with parameters corresponding to signals in signal_train
    :param par_val: numpy array with parameters corresponding to signals in signal_val
    ...
    :param lr_min_delta: float, minimum reduction in validation loss before reducing learning rate
    ...
    See https://keras.io/api/callbacks/reduce_lr_on_plateau/ and https://keras.io/api/callbacks/early_stopping/ for more information about
    the LR scheduler and the Early Stopping callback.
    
    :return: tuple of lists with the form (training loss, validation loss), giving the loss at each epoch
    """
    _compile_model(emulator, initial_lr, relative_mse=True, signal_train=signal_train)  # compiles the emulator with the given LR
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=lr_factor, patience=lr_patience, min_delta=lr_min_delta,
                                  min_lr=min_lr)  # callback for the LR scheduler
    early_stop = EarlyStopping(monitor='val_loss', min_delta=es_delta, patience=es_patience, restore_best_weights=True)  # early stopping
    ...
    X_train, X_val = pp.par_transform(par_train, par_train), pp.par_transform(par_val, par_train)  # input variables
    validation_set = (X_val, y_val)
    hist = emulator.fit(x=X_train, y=y_train, batch_size=256, epochs=epochs, validation_data=validation_set,
                        validation_batch_size=256, callbacks=[reduce_lr, early_stop])  # train emulator
...
def _train_autoencoder(autoencoder, signal_train, signal_val, epochs, initial_lr, lr_factor, lr_patience, lr_min_delta, min_lr,
    ...
    :return: tuple of lists with the form (training loss, validation loss), giving the loss at each epoch
    """
    _compile_model(autoencoder, initial_lr, relative_mse=True, signal_train=signal_train)  # compiles the autoencoder with the given LR
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=lr_factor, patience=lr_patience, min_delta=lr_min_delta,
                                  min_lr=min_lr)  # LR schedule callback
    early_stop = EarlyStopping(monitor='val_loss', min_delta=es_delta, patience=es_patience, restore_best_weights=True)  # early stopping
    y_train, y_val = pp.preproc(signal_train, signal_train), pp.preproc(signal_val, signal_train)  # input and target variables
    ...
    hist = autoencoder.fit(x=y_train, y=y_train, batch_size=256, epochs=epochs, callbacks=[reduce_lr, early_stop],
                           validation_data=validation_set, validation_batch_size=256)  # train autoencoder
    loss, val_loss = hist.history['loss'], hist.history['val_loss']
    return loss, val_loss


def _train_ae_based_emulator(emulator, encoder, signal_train, signal_val, par_train, par_val, epochs, initial_lr, lr_factor, lr_patience,
                    lr_min_delta, min_lr, es_delta, es_patience):
    """
    ...
    :return: tuple of lists with the form (training loss, validation loss), giving the loss at each epoch
    """
    _compile_model(emulator, initial_lr, relative_mse=False, signal_train=None)  # compile the emulator with the given LR
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=lr_factor, patience=lr_patience, min_delta=lr_min_delta,
                                  min_lr=min_lr)  # LR schedule callback
    early_stop = EarlyStopping(monitor='val_loss', min_delta=es_delta, patience=es_patience, restore_best_weights=True)  # early stopping
    y_train = encoder.predict(pp.preproc(signal_train, signal_train))  # autoencoder latent params are target variable
    y_val = encoder.predict(pp.preproc(signal_val, signal_train))  # validation target
    X_train, X_val = pp.par_transform(par_train, par_train), pp.par_transform(par_val, par_train)  # input params
    validation_set = (X_val, y_val)
    hist = emulator.fit(x=X_train, y=y_train, batch_size=256, epochs=epochs, validation_data=validation_set,
                        validation_batch_size=256, callbacks=[reduce_lr, early_stop])  # train emulator
    loss, val_loss = hist.history['loss'], hist.history['val_loss']
...
def train_ae_emulator(autoencoder, encoder, emulator, signal_train, signal_val, par_train, par_val, epochs, ae_lr, em_lr,
    ...
    :param ae_lr_factor: float, factor to multiply learning rate by in LR scheduler of the autoencoder
    :param ae_lr_patience: int, number of epochs to wait before reducing learning rate of the autoencoder
    :param ae_lr_min_delta: float, minimum reduction in validation loss before reducing learning rate of the autoencoder
    :param ae_min_lr: float, minimum allowed learning rate of the autoencoder
    :param ae_es_delta: float, like lr_min_delta but for early stopping of the autoencoder training
    :param ae_es_patience: int, like lr_patience but for early stopping of the autoencoder training
    :param em_lr_factor: float, factor to multiply learning rate by in LR scheduler of the emulator
    :param em_lr_patience: int, number of epochs to wait before reducing learning rate of the emulator
    :param em_lr_min_delta: float, minimum reduction in validation loss before reducing learning rate of the emulator
    :param em_min_lr: float, minimum allowed learning rate of the emulator
    :param em_es_delta: float, like lr_min_delta but for early stopping of the emualtor training
    :param em_es_patience: int, like lr_patience but for early stopping of the emulator training
    
    See https://keras.io/api/callbacks/reduce_lr_on_plateau/ and https://keras.io/api/callbacks/early_stopping/ for more information about
    the LR scheduler and the Early Stopping callback.
    
    :return: tuple of lists with the form (autoencoder training loss, autoencoder validation loss, emulator training loss,
    emulator validation loss), giving the loss at each epoch
    """
