...
def par_transform(parameters, params_train):
    """
    Function that preprocess a set of parameters the same way that the training set parameters are processed:
    that is, take log of first three columns and apply a linear map that makes all the training set parameters
    be in the range [-1, 1]. Note that this map will not send other sets of parameters to [-1, 1].
    ...
    newparams[:, 3:] = parameters[:, 3:].copy()  # copy the rest of the parameters (unchanged)
    ...
    newparams_tr[:, 3:] = params_train[:, 3:].copy()  # copy the rest of the parameters (unchanged)
    ...
    newparams /= (maximum - minimum)  # divide by (max-min) to get the range [0, 1]
    newparams *= 2
